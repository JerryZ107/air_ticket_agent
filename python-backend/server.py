...
class AirlineServer(ChatKitServer[dict[str, Any]]):
    ...
    async def respond(
        ...
        if user is not None:
            state.context.user_id = str(user.id)
            state.context.username = user.username
            state.context.user_role = user.role
            state.context.passenger_name = user.display_name or user.username
            state.context.trace_id = context.get("trace_id")
        ...
        if user_text:
            ...
            if (
                ...
                and user_text
                and state.context.username
            ):
                from pipeline.admin_customer import answer_admin_customer_lookup

                admin_reply = await answer_admin_customer_lookup(
                    user_text, state.context.username
                )
                if admin_reply:
            ...
            if (
                state.context.user_role == "admin"
                and "订单" in user_text
                and any(k in user_text for k in ("最近", "列出", "全库", "全部"))
                and "旅客" not in user_text
            ):
                state.current_agent_name = "Booking and Cancellation Agent"

            if (
                route.confidence >= 0.85
                and route.target_agent == "FAQ Agent"
            ):
                from pipeline.faq_direct import answer_from_manual

                reply = await answer_from_manual(user_text)
            ...
            if not agent_breaker.allow():
                busy = "系统繁忙，请稍后再试。"
                state.input_items.append({"role": "assistant", "content": busy})
                yield ThreadItemDoneEvent(
                    item=AssistantMessageItem(
                        id=self.store.generate_item_id("message", thread, context),
                        thread_id=thread.id,
                        created_at=datetime.now(),
                        content=[AssistantMessageContent(text=busy)],
                    )
                )
                return

        if state.context.user_id:
            from pipeline.context_manager import compress_if_needed

            state.input_items = await compress_if_needed(
                state.input_items,
                thread_id=thread.id,
        ...
        try:
            ...
            async for event in stream_agent_response(chat_context, result):
                ...
                if hasattr(event, "item"):
                    try:
                        ...
                        logging.getLogger("airline.runner").exception("failed to record run event")
                yield event
            ...
            await agent_breaker.record_failure({"reason": "max_turns"})
            await self._broadcast_state(thread, context)
            return
            ...
            await agent_breaker.record_failure({"reason": "agent_run"})
            raise
        await agent_breaker.record_success()
