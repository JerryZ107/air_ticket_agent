...
class MemoryStore(Store[dict[str, Any]]):
    ...
    会话按登录用户隔离：thread 创建时记录归属 user_id，后续读取校验归属，
    不匹配一律抛 NotFoundError（与"不存在"同义，避免泄露会话存在性）。
    """

    def __init__(self) -> None:
        self._threads: Dict[str, _ThreadState] = {}
        self._attachments: Dict[str, Attachment] = {}
        self._thread_owners: Dict[str, str] = {}  # thread_id -> str(user_id)

    @staticmethod
    def _owner_from_context(context: dict[str, Any]) -> str | None:
        user = context.get("user") if isinstance(context, dict) else None
        user_id = getattr(user, "id", None)
        return str(user_id) if user_id else None

    def _assert_owner(self, thread_id: str, context: dict[str, Any]) -> None:
        owner = self._thread_owners.get(thread_id)
        if owner is None:
            return
        current = self._owner_from_context(context)
        # 无法解析当前用户时放行（内部/非 HTTP 调用）；能解析则必须一致
    ...
    async def save_thread(self, thread: ThreadMetadata, context: dict[str, Any]) -> None:
        ...
        owner = self._owner_from_context(context)
        if owner is not None:
            existing = self._thread_owners.get(thread.id)
            if existing is not None and existing != owner:
                raise NotFoundError(f"Thread {thread.id} not found")
            self._thread_owners[thread.id] = owner
    ...
    async def load_threads(
        ...
        threads = sorted(
            (
                ...
                if owner is None or self._thread_owners.get(tid) == owner
            ),
