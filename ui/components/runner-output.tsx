...
import { agentDisplayName } from "@/lib/i18n";
import {
  ArrowRightLeft,
  Wrench,
  WrenchIcon,
...
function buildEventText(event: AgentEvent) {
  switch (event.type) {
    case "handoff":
      return event.content || `${agentDisplayName(event.metadata?.source_agent ?? "")} -> ${agentDisplayName(event.metadata?.target_agent ?? "")}`.trim();
    case "tool_call": {
      const args = event.metadata?.tool_args;
      const argsText = args !== undefined ? ` - ${inlineValue(args)}` : "";
      return `${event.content || "工具调用"}${argsText}`;
    }
    case "tool_output": {
      const result = event.metadata?.tool_result;
      if (result !== undefined) return inlineValue(result);
      return event.content || "工具结果";
    }
    case "context_update": {
          ...
          <div className="p-3 space-y-2.5">
            {runnerEvents.length === 0 ? (
              ...
              groupedEvents.map((group) => {
                const agentName = agentDisplayName(group[0]?.agent ?? "Agent");
